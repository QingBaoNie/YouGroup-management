...
class AutoRecallKeywordPlugin(Star):
    ...
    def load_json_data(self):
        with open('cesn_data.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
            self.kick_black_list.update(data.get('kick_black_list', []))
            self.target_user_list.update(data.get('target_user_list', []))
            self.sub_admin_list.update(data.get('sub_admin_list', []))
    ...
    async def auto_recall(self, event: AstrMessageEvent):
        ...
        if len(self.user_message_times[key]) == self.spam_count and now - self.user_message_times[key][0] <= self.spam_interval:
    ...
    async def handle_commands(self, event: AstrMessageEvent):
        ...
        at_list = [getattr(seg, 'qq', None) for seg in getattr(event.message_obj, 'message', []) if getattr(seg, 'type', '') == 'At']
